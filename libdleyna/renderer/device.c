			    ...
			    GVariantBuilder *changed_props_vb)
{
	...
	if (!seek && (timeseek || byteseek))
		DLEYNA_LOG_WARNING("Malformed Transport Action X_DLNA_Seek* provided but seek is missing")
	...
	val = seek && (timeseek || byteseek) ? true_val : false_val;
	g_variant_ref(val);
	prv_change_props(device->props.player_props,
			 DLR_INTERFACE_PROP_CAN_SEEK, val,
			 changed_props_vb);

	val = seek && timeseek ? true_val : false_val;
	g_variant_ref(val);
	prv_change_props(device->props.player_props,
			 DLR_INTERFACE_PROP_CAN_TIME_SEEK, val,
			 changed_props_vb);
			    ...
			    GVariantBuilder *changed_props_vb)
{
	...
	prv_change_props(device->props.player_props,
			 DLR_INTERFACE_PROP_COUNTER_POSITION, val,
			 changed_props_vb);
}
				     ...
				     gpointer user_data)
{
	...
	GVariant *changed_props;
					     ...
					     &rel_cnt, NULL);
	if (!end || ((rel_pos == NULL) && (rel_cnt == NULL))) {
			 ...
			 dlr_upnp_task_complete_t cb)
{
	...
	if ((!strcmp(get_prop->interface_name, DLR_INTERFACE_PLAYER) ||
	    ...
	    (!strcmp(task->ut.get_prop.prop_name,
	     ...
	     !strcmp(task->ut.get_prop.prop_name,
			DLR_INTERFACE_PROP_COUNTER_POSITION))) {
				    ...
				    dlr_upnp_task_complete_t cb)
{
	...
	if (!strcmp(pos_type, "TRACK_NR"))
		...
		position = g_strdup_printf("%llu", seek_data->counter_position);
